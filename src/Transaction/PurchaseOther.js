...
const PurchaseOtherEntry = () => {
    ...
    try {
      ...
      purchaseEntryDta.map(async (purchase) => {
        try {
          ...
          console.error(`Failed to fetch account for purchase other ${purchase.Id}:`, error);
            ...
            <Button
              ...
              onClick={() => {
                const invdetail = purchasedetails
                  .filter((detail) => detail.PurchaseotherId === row.original.Id)
                  .map((detail) => {
 ...
 const logoDiameter = 20; // Diameter of the circle
  ...
  y += 6;
    ...
    // // Split layout: Left (Details & Tax) and Right (Summary)
    // const leftStartY = y;
    // let rightStartY = y;

    // // Left: Purchase Details
    // doc.setFontSize(11);
    // doc.text("Purchase Details", 10, leftStartY);
    // let finalY = leftStartY + 2;
    ...
    // doc.text("Tax Details", 10, finalY);

    // autoTable(doc, {
    //   head: [["CGST%", "CGST Amount", "SGST%", "SGST Amount", "IGST%", "IGST Amount"]],
    //   body: previewData.invdetail.map(i => [i.CGSTPercentage, i.CGSTAmount, i.SGSTPercentage, i.SGSTAmount, i.IGSTPercentage, i.IGSTAmount]),
    //   startY: finalY + 5,
    //   margin: { left: 10, right: 110 },
    //   theme: "grid",
    //   styles: { fontSize: 9 },
 ...
 doc.setFontSize(12);
  ...
  y = doc.lastAutoTable.finalY + 10;
    ...
    // Save
    doc.save("Purchase_Other_Preview.pdf");
  };
        ...
        <Box>
          ...
          <Dialog open={previewOpen} onClose={() => setPreviewOpen(false)} maxWidth="xlg" fullWidth>
           <DialogTitle sx={{ textAlign: 'center' }}>
              ...
              <Typography variant="h6" mt={1} ><b>Purchase Other Entry Preview</b>  </Typography>
            </DialogTitle>
                  ...
                  <Box>
                    ...
                    <Grid container spacing={4}>
                      ...
                      <Grid item xs={12} md={8}>
                        ...
                        <TableContainer component={Paper} elevation={2} sx={{ width: '100%', mt: 1 }}>
                          <Table size="small">
                            <TableHead sx={{ backgroundColor: '#f5f5f5' }}>
                              <TableRow>
         ...
         <TableContainer component={Paper} elevation={2} sx={{ width: '100%', mt: 2 }}>
                        ...
                        <TableContainer component={Paper} elevation={2} sx={{ width: '100%', mt: 4 }}>
                          <Table size="small">
                            <TableHead sx={{ backgroundColor: '#f5f5f5' }}>
                              <TableRow>
                      ...
                      {/* Right Side - Vertical Summary Table */}
