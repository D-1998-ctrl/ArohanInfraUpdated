...
//   DialogTitle, IconButton, Grid, Menu, Table,  TableBody, TableCell, TableContainer, TableHead, TableRow,
//   Paper,  Box, useMediaQuery, Button, Typography, TextField, Drawer, Divider, FormControl, MenuItem, FormControlLabel, Checkbox
// }
//   from '@mui/material';
...
//         `https://arohanagroapi.microtechsolutions.net.in/php/get/gettblpage.php?Table=purchaseheader&PageNo=${pageNo}`,
//         requestOptions
//       );
...
//     try {
//       const response = await axios.get(
//         "https://arohanagroapi.microtechsolutions.net.in/php/get/gettable.php?Table=purchasedetail"
//       );
...
//         ? "https://arohanagroapi.microtechsolutions.net.in/php/updatepurchaseheader.php"
//         : "https://arohanagroapi.microtechsolutions.net.in/php/postpurchaseheader.php";
...
//           row.Id
//             ? "https://arohanagroapi.microtechsolutions.net.in/php/updatepurchasedetail.php"
//             : "https://arohanagroapi.microtechsolutions.net.in/php/postpurchasedetail.php";
...
//     fetch(`https://arohanagroapi.microtechsolutions.net.in/php/delete/deletetable.php?Table=PurchaseHeader&Id=${rowId}`, requestOptions)
  ...
  const columns = useMemo(() => {
    return [
...

      {
        header: "Party Name",
        size: 100,
        accessorFn: (row) => row.accountDetails?.AccountName || "Null",
        ...
        Cell: ({ row }) => (
          <Box display="flex" gap={1}>
            ...
            sx={{ background: 'var(--complementary-color)'}}
              ...
              onClick={() => {
                const invdetail = purchasedetails
                  .filter((detail) => detail.PurchaseId === row.original.Id)
                  .map((detail) => {
                    const matchedMaterial = productOptions.find(
                      (item) => item.value.toString() === detail.MaterialId?.toString()
                    );
                    return {
                      ...detail,
                      MaterialName: matchedMaterial?.label || "Unknown",
                    };
                  });
                ...
                {previewData ? (
                  <Box>
                    ...
                    <Box display={'flex'} justifyContent={'space-between'} gap={2}>
                      <Typography><strong>Purchase No:</strong> {previewData.PurchaseNo}</Typography>
                      <Typography>
                        <strong>Purchase Date:</strong>{" "}
                        {new Date(previewData.PurchaseDate.date).toLocaleDateString()}
                      </Typography>
                    </Box>
                  ...
                  <Divider sx={{ mt: 2 }} />
                    ...
                    {/* <Grid container spacing={4}>
                      ...
                      <Grid item xs={12} md={8}>
                            ...
                            <TableBody>
                              {previewData.invdetail.map((item, index) => (
                                <TableRow key={index}>
                                  ...
                                  <TableCell>{item.IGSTPercentage}</TableCell>
                        ...
                        <TableContainer component={Paper} elevation={2} sx={{ width: '100%', mt: 8 }}>
                          <Table size="small">
                            <TableHead sx={{ backgroundColor: '#f5f5f5' }}>
                              <TableRow>
